...
<?= $this->section('content') ?>
  ...
  <div class="container">
    <h3 class="text-center mb-5" data-aos="fade-up"><?= esc($desk['title'] ?? 'President\'s Message') ?></h3>
    <div class="card border-0 shadow-lg" data-aos="fade-up" data-aos-delay="100">
      <div class="row g-0">
        <div class="col-lg-5 d-flex flex-column align-items-center justify-content-center bg-white p-4">
          <img src="<?= base_url('public/uploads/' . ($desk['image'] ?? 'default.jpg')) ?>" alt="President" class="img-fluid rounded shadow mb-3">
          <div class="text-center">
            <p class="mb-1"><strong><?= esc($desk['name'] ?? 'President Name') ?></strong></p>
            <p class="mb-1"><strong><?= esc($desk['designation'] ?? 'Founder President') ?></strong></p>
            <p class="mb-0"><?= esc($desk['address'] ?? 'Organization') ?></p>
          </div>
        </div>
        <div class="col-lg-7">
          <div class="card-body p-4">
            <h4 class="card-title mb-3"><?= esc($desk['subtitle'] ?? 'From The President\'s Desk') ?></h4>
            <?= $desk['overview'] ?? '<p>No message available.</p>' ?>
