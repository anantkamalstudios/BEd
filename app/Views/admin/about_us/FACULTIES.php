...
<div class="page-inner">
    <h1 class="mb-4">Teaching Faculty Page</h1>
...
<br>
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="page-title">Teaching Faculty</h2>
   ...
   <table class="table table-bordered">
        ...
        <?php foreach ($teaching as $row): ?>
            <tr>
                ...
                <td>
                    ...
                    <a href="<?= base_url('delete_faculty/' . $row['id']) ?>" 
            ...
            <div class="modal fade" id="editTeachingModal<?= $row['id'] ?>" tabindex="-1" aria-labelledby="editModalLabel<?= $row['id'] ?>" aria-hidden="true">
                <div class="modal-dialog modal-lg">
                    <form action="<?= base_url('update_teaching_faculty') ?>" method="post" enctype="multipart/form-data">
                        <?= csrf_field() ?>
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body row">
                                ...
                                <div class="form-group col-md-6 mb-3">
                                    ...
                                    <input type="text" name="qualification" class="form-control" value="<?= esc($row['qualification']) ?>" required>
                                </div>
                                <div class="form-group col-md-6 mb-3">
                                    <label>Image</label>
                                    <input type="file" name="image" class="form-control">
                                    <?php if (!empty($row['image'])): ?>
                                        <img src="<?= base_url('public/uploads/' . $row['image']) ?>" width="100" class="mt-2 img-thumbnail">
                                    <?php endif; ?>
                                </div>
                            </div>
    ...
    <div class="modal fade" id="heroModal" tabindex="-1" aria-labelledby="heroModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
        <form action="<?= base_url('save_teaching_faculty') ?>" method="post" enctype="multipart/form-data">
                <?= csrf_field() ?>
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h2 class="page-title">Non-Teaching Faculty</h2>
        <button class="btn btn-success" data-bs-toggle="modal" data-bs-target="#nonheroModal">Add New</button>
    </div>
    <!-- add faculty -->
       <table class="table table-bordered">
    <thead>
    ...
    <tbody>
        <?php $j = 1; ?>
        <?php foreach ($nonteaching as $row): ?>
            <tr>
                <td><?= $j++ ?></td>
                <td><?= esc($row['designation']) ?></td>
                <td><?= esc($row['name']) ?></td>
                ...
                <td>
                    <button class="btn btn-sm btn-primary" 
                            data-bs-toggle="modal" 
                            data-bs-target="#editNonTeachingModal<?= $row['id'] ?>"><i class="fas fa-edit"></i></button>
                    <a href="<?= base_url('delete_non_faculty/' . $row['id']) ?>" 
                       onclick="return confirm('Are you sure?')" 
                       class="btn btn-sm btn-danger"><i class="fas fa-trash-alt"></i></a>
                </td>
            </tr>
            ...
            <!-- Edit Modal -->
            <div class="modal fade" id="editNonTeachingModal<?= $row['id'] ?>" tabindex="-1" aria-labelledby="editModalLabel<?= $row['id'] ?>" aria-hidden="true">
                <div class="modal-dialog modal-lg">
                    <form action="<?= base_url('update_non_teaching_faculty') ?>" method="post" enctype="multipart/form-data">
                        <?= csrf_field() ?>
                        <input type="hidden" name="id" value="<?= $row['id'] ?>">
                        <div class="modal-content">
                            <div class="modal-header">
                                <h5 class="modal-title">Edit Non-Teaching Faculty</h5>
                                <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                            </div>
                            <div class="modal-body row">
                                <div class="form-group col-md-6 mb-3">
    ...
    <!-- Modal -->
    <div class="modal fade" id="nonheroModal" tabindex="-1" aria-labelledby="nonheroModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
        <form action="<?= base_url('save_non_teaching_faculty') ?>" method="post" enctype="multipart/form-data">
                <?= csrf_field() ?>
                <input type="hidden" name="id" id="hero_id">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="nonheroModalLabel">Add/Edit Teaching Faculty</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"></button>
                    </div>
                    <div class="modal-body row">
                        <div class="form-group col-md-6 mb-3">
