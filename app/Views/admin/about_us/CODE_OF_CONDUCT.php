...
<div class="page-inner">
    ...
    <tbody>
        <?php if (!empty($code_list)) : ?>
            <?php $i = 1; foreach ($code_list as $row) : ?>
                <tr>
                    <td><?= $i++ ?></td>
                    <td>
                    ...
                    <td>
                        <button class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#heroModal"
                            onclick="editCoc(<?= $row['id'] ?>, '<?= esc($row['file_name']) ?>')"><i class="fas fa-edit"></i></button>

                        <a href="<?= base_url('delete_coc/' . $row['id']) ?>" 
    ...
    <div class="modal fade" id="heroModal" tabindex="-1" aria-labelledby="heroModalLabel" aria-hidden="true">
        ...
        <form action="<?= base_url('insert_coc') ?>" method="post" enctype="multipart/form-data">
                <?= csrf_field() ?>
                <input type="hidden" name="id" id="hero_id">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="heroModalLabel">Add/Edit CODE OF CONDUCT</h5>
